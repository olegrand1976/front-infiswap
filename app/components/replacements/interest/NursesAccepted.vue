...
const refresh = async (newPage: number) => {
    page.value = newPage;
    await nursesAccepted(page.value, perPage.value, {
        sortKey: sort.by,
...
const openModal = (row: NurseAcceptedRow) => {
...
const columns: ColumnDef<NurseAcceptedRow>[] = [
    {
        ...
        cell: info =>
            h('div', { class: 'py-2 ml-3' }, [
                h(UsersName, { user: info.row.original.user }),
            ]),
    },
    {
