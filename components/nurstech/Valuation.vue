<template>
    <section class="container mx-auto px-4 py-10">
        <div class="flex flex-col lg:flex-row">
            <div class="lg:w-1/2 w-full">
                <LayoutsAppImage
                    src="/home/hand_on_a_leaf.png"
                    alt="Image"
                    class="w-full h-[320px] lg:w-[700px] lg:h-[550px]"
                />
            </div>

            <div class="lg:w-1/2 w-full p-6 pl-10">
                <p class="text-lg font-bold text-primary mb-4">
                    Infirmier·ère libéral·e ?
                </p>
                <p class="mb-4">
                    Créez votre page web professionnelle avec <span class="font-bold text-primary">NURS</span><span class="font-bold text-primarytech">TECH</span><br>
                    Valorisez votre activité et attirez plus de patients à domicile grâce
                    à une page personnalisée, visible sur Infiswap.
                </p>

                <p class="text-lg font-semibold text-primary mb-4">
                    Votre métier est essentiel. Votre présence en ligne aussi.
                </p>
                <p class="mb-4">
                    Avec NURSTECH, bénéficiez d’une page web claire, professionnelle
                    et optimisée pour que vos futurs patients vous trouvent facilement
                    et vous contactent rapidement.
                </p>
                <h2 class="text-lg font-bold text-primary mb-4">
                    Ce que vous gagnez :
                </h2>
                <ul class="space-y-2">
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Une page web professionnelle à votre image
                    </li>
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Mise en avant de vos services à domicile
                    </li>
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Horaires, secteurs d’intervention et spécialités bien présentés
                    </li>
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Formulaire de contact intégré pour faciliter les demandes de soins
                    </li>
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Compatible mobile pour consultation rapide
                    </li>
                    <li class="flex items-start">
                        <span class="mr-2 text-green-600">✅</span>
                        Création rapide : vous répondez à un questionnaire, on s’occupe du reste
                    </li>
                </ul>
            </div>
        </div>
    </section>
</template>
